...
if 'dossier_unlocked' not in st.session_state: 
...
with st.sidebar:
    ...
    if st.session_state.sys_state != "OPERATOR_LOGIN":
        ...
        st.markdown("<div class='sidebar-header'>🗄️ OPERATIONS</div>", unsafe_allow_html=True)
        if st.button("TERMINAL HOME", key="nav_home"): nav_to("TERMINAL_HOME"); st.rerun()
        if st.button("AM INTERCEPT", key="nav_mw"): nav_to("MW_LOG"); st.rerun()
        if st.button("FM INTERCEPT", key="nav_fm"): nav_to("FM_LOG"); st.rerun()
        if st.button("NWR INTERCEPT", key="nav_nwr"): nav_to("NWR_LOG"); st.rerun()
        if st.button("BULK INFILTRATION", key="nav_bulk"): nav_to("BULK_IMPORT"); st.rerun()
...
with main_content:
    ...
    if st.session_state.sys_state == "OPERATOR_LOGIN":
        ...
        with st.form("login_form"):
            ...
            # Identify current country index for fallback
    ...
    # --- 8E. THE CLANDESTINE MATRIX (BOUNTY ROOM) ---
        ...
        * Acquisition window closes exactly 14 days from transmission.
        ...
        .dossier-stamp { color: #ff0000; font-size: 3rem; font-weight: bold; text-transform: uppercase; border: 4px solid #ff0000; display: inline-block; padding: 5px 15px; transform: rotate(-5deg); margin-bottom: 15px; text-shadow: 0px 0px 10px rgba(255,0,0,0.5); }
        </style>
        """, unsafe_allow_html=True)
    ...
    # Stubs for the remaining operational modules (until we build them)
    elif st.session_state.current_page == "BULK_IMPORT":
        st.markdown(f"<h1>{st.session_state.current_page.replace('_', ' ')} MODULE</h1>", unsafe_allow_html=True)
        st.warning("MODULE OFFLINE. AWAITING DEPLOYMENT.")
